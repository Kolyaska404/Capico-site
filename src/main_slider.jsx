...
export function Slider_1() {
    ...
    return (
        <>
            <nav className="flex gap-x-[22px] absolute right-[40px] z-10">
                <button onClick={() => setActiveID(false)} className="border-white border-[1px] rounded-[70px] w-[94px] h-[94px] flex items-center justify-center hover:border-[#7952FF] transition-colors"><img src="/main_page/arrow_left.svg" alt="" /></button>
                <button onClick={() => setActiveID(true)} className="border-white border-[1px] rounded-[70px] w-[195px] h-[94px] flex items-center justify-center hover:border-[#7952FF] transition-colors"><img src="/main_page/arrow_right.svg" alt="" /></button>
            </nav>
            <ul className={`flex gap-x-[60px] ml-[165px] pt-[115px] mb-[53px] transition-transform ease-in ${activeID ? 'translate-x-[-990px]' : 'translate-x-0'}`}>
                {list.map((slides) => (
